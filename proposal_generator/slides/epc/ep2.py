...
Design v2 "Institutional Trust Grid":
  ...
  Band 2: ownership feature notes on a C_PANEL band
"""
...
FLOW_STEPS = [
    ("1", "設備購入", "お客様が設備を購入・所有"),
    ("2", "施工・設置", "EPC事業者が設計・施工を実施"),
    ("3", "自家消費", "発電電力をお客様が100%利用"),
    ("4", "電気代削減", "電力会社への支払いが大幅に減少"),
]

FEATURES = [
    ("設備はお客様の資産",
     "太陽光パネル・PCS等の設備はお客様が購入し、自社資産として計上できます。"),
    ("減価償却メリット",
     "法定耐用年数17年で減価償却。税制優遇で即時償却も可能です。"),
    ("長期コスト削減",
     "PPA単価の支払いが不要。発電した電力は全て無料で利用できます。"),
    ("契約期間の自由度",
     "PPAのような長期契約期間の縛りがなく、自由に運用できます。"),
]
...
def generate(slide, data: dict, logo_path: Path = None) -> None:
    ...
    # --- Band 1: 4-step flow ---
    y = ys[0]
    add_section_header(slide, MARGIN, y, content_w, "EPC導入のしくみ（4ステップ）")
    y += header_h
