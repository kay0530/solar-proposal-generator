"""
ep5.py - デマンドカット試算（EPC）

Same logic as PP9 but titled for EPC model.
...
def generate(slide, data: dict, logo_path: Path = None) -> None:
    ...
    if chart_before and chart_after:
        ...
        chart_h = (SLIDE_H - y - Inches(0.5)) / 2
...
def _add_demand_chart(slide, x, y, w, h, title: str,
    ...
    series_demand.format.line.color.rgb = RGBColor(0x00, 0x20, 0x60)
    ...
    # Convert self_c series into areaChart (filled area under line)
    try:
        ...
        _areaChart = _etree.fromstring(
            f'<c:areaChart xmlns:c="{_ns}" xmlns:a="{_a_ns}">'
            f'<c:grouping val="standard"/><c:varyColors val="0"/>'
            f'</c:areaChart>'
        )
        _spPr_new = _etree.fromstring(
    ...
    # cat_axis.tick_label_position left at default
    try:
